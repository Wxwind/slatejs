import { Entity, THREE, TransformUtils } from 'deer-engine';
...
import { getAABBInfo, findLineIntersection3D, abs } from '../../util';
import { SphereColliderObj } from '@/editor-engine/module/three-collider';

export class SphereEditHelper implements IEditHelper {
  ...
  constructor(
    ...
    private object: SphereColliderObj
  ) {
  ...
  onLeave(): void {
    ...
    console.log('leave editor sphere');
  ...
  handleMouseMove = (event: MouseEvent) => {
    ...
    if (activePoint) {
      const { dragParams, originalPos } = activePoint.userData as ControlPointUserData;
      const { initSize, initScale, fixedPointWorldPos, pointSign } = dragParams;
      ...
      // 球碰撞体不能是椭球，所以要纠偏
      // 求originalPos到fixPointWorldPos的直线 在intersection垂直向下的直线 在无视z轴下的交点，且交点在前者的直线上

      const p4 = _tempVec2.copy(intersection);
      p4.y += 1;
      const fixed = findLineIntersection3D(originalPos, fixedPointWorldPos, intersection, p4);
      if (fixed === null) {
        throw new Error('计算有误,交点不存在');
      }

      intersection.copy(fixed);
