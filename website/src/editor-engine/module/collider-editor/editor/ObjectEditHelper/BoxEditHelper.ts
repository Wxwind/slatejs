import { Entity, IVector3, THREE, TransformUtils } from 'deer-engine';
...
import { BoxColliderObj } from '@/editor-engine/module/three-collider';

export class BoxEditHelper implements IEditHelper {
  ...
  handleMouseMove = (event: MouseEvent) => {
    ...
    if (activePoint) {
      const { initSize, initScale, pointSign, fixedPointWorldPos } = (activePoint.userData as ControlPointUserData)
        .dragParams;
      ...
      // 防止颠倒
      if (pointSign.x * (intersection.x - fixedPointWorldPos.x) < 0) {
        intersection.x = fixedPointWorldPos.x + pointSign.x * 0.1;
      }
      if (pointSign.y * (intersection.y - fixedPointWorldPos.y) < 0) {
        intersection.y = fixedPointWorldPos.y + pointSign.y * 0.1;
      }
      if (pointSign.z * (intersection.z - fixedPointWorldPos.z) < 0) {
        intersection.z = fixedPointWorldPos.z + pointSign.z * 0.1;
      }

      // const deltaCoords = intersection.sub(startPosition);

      // const u2 = deltaCoords.y;
      // const u1 = diagonalVector.clone().normalize().dot(deltaCoords);

      // const angle = Math.atan(initSize.z / initSize.x);

      // const deltaZ = u1 * Math.sin(angle);
      // const deltaX = u1 * Math.cos(angle);
      // const deltaHeight = u2;

      // const newSize = initSize.clone().add({ x: deltaX, y: deltaHeight, z: deltaZ });

      // activePoint.position.copy(fixPointWorldPos).add(newSize.clone().multiply(pointSign));

      // const newCenter = _tempVec1
      //   .copy(activePoint.position)
      //   .add(fixPointWorldPos)
      //   .multiplyScalar(0.5);
  ...
  private saveSettings() {
    const { owner, object, _tempVec2 } = this;
    ...
    const realSize = IVector3.copy(_tempVec2, object.size).multiply(scale).divide(worldScale);
