import { Fsm, genUUID, isNil, THREE, TransformUtils } from 'deer-engine';
...
import { getAABBInfo, intersectObject } from '../../util';

export class BoxColliderCreator extends ColliderToolBase {
  readonly name: ColliderToolType = 'Box';
  ...
  override onLeave(fsm: Fsm<ColliderEditor>): void {
    ...
    // this.owner.component.scene.getManager(SelectManager)?.updateBoundingBox();
    if (this.object) {
  ...
  handleMouseDown = (event: MouseEvent) => {
    ...
    const { raycaster, owner, rootObject, virtualPlane, startPosition } = this;
  ...
  handleMouseMove = (event: MouseEvent) => {
    ...
    if (deltaCoords.length() > threshold) {
      ...
      object.scale.set(Math.abs(deltaPosition.x), Math.abs(deltaPosition.x), Math.abs(deltaPosition.z));
      object.position
        .copy(startPosition)
        .add(new THREE.Vector3(deltaPosition.x / 2, Math.abs(deltaPosition.x / 2), deltaPosition.z / 2));
    } else {
  ...
  private saveSettingsByDefault = () => {
    const { owner, startPosition } = this;
    if (isNil(startPosition)) return;
