import { Fsm, genUUID, THREE, TransformUtils } from 'deer-engine';
...
export class CapsuleColliderCreator extends ColliderToolBase {
  readonly name: ColliderToolType = 'Capsule';
  ...
  createObject(radius: number, height: number) {
  ...
  handleMouseDown = (event: MouseEvent) => {
    ...
    if (intersects.length > 0) {
      ...
      this.startPlane.setFromNormalAndCoplanarPoint(new THREE.Vector3(1, 0, 1), startPosition);
    } else {
      const intersectPos = raycaster.ray.intersectPlane(virtualPlane, startPosition);
      console.log('相交于y=0', intersectPos);

      if (intersectPos) {
        this.startPlane.setFromNormalAndCoplanarPoint(new THREE.Vector3(1, 0, 1), intersectPos);
      }
    }
  ...
  handleMouseMove = (event: MouseEvent) => {
    ...
    if (deltaCoords.length() > threshold) {
      // xz平面等比例缩放,以xz中较大者为基准
      if (group.visible === false) {
        group.visible = true;
      }
      const radius = Math.max(Math.abs(deltaPosition.x), Math.abs(deltaPosition.z)) / 2;
      const totalLength = Math.abs(deltaPosition.y);
      const height = Math.max(totalLength - 2 * radius, 0);

      group.remove(object);
      const newObject = this.createObject(radius, height);
      group.add(newObject);
      ...
      newObject.position
        ...
        .add(
          new THREE.Vector3(
            radius * Math.sign(deltaPosition.x),
            (totalLength / 2) * Math.sign(deltaPosition.y),
            radius * Math.sign(deltaPosition.z)
          )
        );
    } else {
      group.visible = false;
  ...
  handleMouseUp = (event: MouseEvent) => {
    const { owner, _deltaCoords, threshold, object, group } = this;
  ...
  private saveSettingsByObject = () => {
    ...
    const maxWorldXZSize = Math.max(worldScale.x, worldScale.z);
    const geo = this.object.geometry as THREE.CapsuleGeometry;

    const config: ColliderConfig = {
      id: genUUID(),
      name: '胶囊体',
      type: 'Capsule',
      radius: geo.parameters.radius / maxWorldXZSize,
      height: geo.parameters.length / worldScale.y,
