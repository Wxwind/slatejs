...
const Timeline: FC<TimelineProps> = (props) => {
  ...
  const handleTimelineScaled = useCallback((e: WheelEvent) => {
    if (e.altKey) {
      e.preventDefault();
      const newScale = Math.max(2, scale + e.deltaY / 10);
      setScale(newScale);
      signals.timelineScaled.emit(scale);
    }
  }, []);

  const handleClickTimeCanvas = useCallback((e: React.MouseEvent<HTMLCanvasElement, MouseEvent>) => {
    e.preventDefault();
  ...
  const updateMarks = useCallback(() => {
    ...
    for (let i = 0.5; i <= width; i += scale) {
      ...
      if (scale > 16) ctx.moveTo(i + scale4 * 1, 22), ctx.lineTo(i + scale4 * 1, 26);
      if (scale > 8) ctx.moveTo(i + scale4 * 2, 22), ctx.lineTo(i + scale4 * 2, 26);
      if (scale > 16) ctx.moveTo(i + scale4 * 3, 22), ctx.lineTo(i + scale4 * 3, 26);
    }
  ...
  const handleTimelineScaledSignal = useCallback((value: number) => {
    setScale(value);
  ...
  return (
    <div className="timeline" ref={timelineRef} onWheel={handleTimelineScaled}>
